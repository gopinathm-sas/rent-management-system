...
export default function RentDetails() {
    ...
    const handleCellClick = async (roomData, monthIndex, currentStatus) => {
        ...
        if (nextStatus === 'Paid' && isFirstOccupancyMonth(tenantData, year, monthIndex)) {
            ...
            if (joinDay > 1) {
                // Calculate water amount for display in dialog
                const waterRate = (tenantData?.waterRate !== null && tenantData?.waterRate !== undefined && String(tenantData?.waterRate).trim() !== '')
                ...
                const waterAmount = (Number.isFinite(water?.amount) ? water.amount : 0) + RENT_WATER_SERVICE_CHARGE;

                // Show prorated rent dialog instead of directly updating
                setProrateDialog({
                    roomData,
                    tenant: tenantData,
                    year,
    ...
    const handleProrateConfirm = async (deductionDays) => {
    ...
    return (
        <>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mt-8">
                    <div className="p-6 border-b border-slate-100 bg-emerald-50/50">
                        ...
                        <p className="text-sm text-emerald-700 mt-1">
                            Rent and Water Details (Water Calculation Disabled Temporarily for Debugging)
                        </p>
                    </div>
                                        ...
                                        <tr key={room.roomId} className="hover:bg-slate-50 transition">
                                            ...
                                            {Array.from({ length: 12 }).map((_, i) => {
                                                ...
                                                if (tenant) {
                                                    ...
                                                    // DEBUG: Bypass computeWaterForMonth
                                                    const waterTotal = RENT_WATER_SERVICE_CHARGE;
                                                    ...
                                                    if (status === 'Paid') {
                                                        if (isEvictionMonth) {
                                                            ...
                                                            if (Number.isFinite(total)) {
                                                                ...
                                                                // Display effective rent + simplified water
            ...
            {/* Prorated Rent Dialog */}
            {prorateDialog && (
                <ProratedRentDialog
                    tenant={prorateDialog.tenant}
                    year={prorateDialog.year}
                    monthIndex={prorateDialog.monthIndex}
                    waterAmount={prorateDialog.waterAmount}
                    onConfirm={handleProrateConfirm}
                    onCancel={() => setProrateDialog(null)}
                />
            )}
        </>
    );
